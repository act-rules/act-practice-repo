...
function validateRuleFrontmatter({ frontmatter }, metaData) {
  ...
  if (rule_type.toLowerCase() === `atomic`) {
    test("has optional property `input_aspects` when `rule_type = atomic`", () => {
      ...
      expect(
        Array.isArray(frontmatter.input_aspects),
        `input_aspects should be a list`
      ).toBe(true);
      expect(
        frontmatter.input_aspects.length > 0,
        `should have atleast one item in input_aspects`
      ).toBe(true);
  ...
  test.each([...authors, ...previous_authors])(
    "has contributor data for author: `%s`",
    author => {
  ...
  const orderedPreviousAuthors = [...previous_authors].sort((a, b) =>
    a.localeCompare(b)
  );
  ...
  if (accessibility_requirements) {
    ...
    test.each(accRequirementValues)(
      "has expected keys for accessibility requirement: `%p`",
      accReq => {
        if (accReq.secondary) {
        ...
        expect(keys).toIncludeAllMembers([
          "failed",
          "forConformance",
          "inapplicable",
          "passed"

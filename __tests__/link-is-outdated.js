...
const badLinksAndRecommendations = {
  "://www.w3.org/TR/WCAG20/":
    "Use WCAG 2.1 reference- https://www.w3.org/WAI/WCAG21/",
  "://www.w3.org/TR/UNDERSTANDING-WCAG20/":
    "Use WCAG 2.1 reference - https://www.w3.org/WAI/WCAG21/Understanding/",
  "://www.w3.org/TR/WCAG20-TECHS/":
    "Use WCAG 2.1 reference - https://www.w3.org/WAI/WCAG21/Techniques/",
  "://www.w3.org/TR/wai-aria-1.0/":
    "Use ARIA 1.1 reference - https://www.w3.org/TR/wai-aria-1.1/",
...
function validateIfLinksAreOutdated({ markdownAST, frontmatter }) {
   ...
   * -> eg: [Alpha](https://....)
   */
  const pageLinks = getMarkdownAstNodesOfType(markdownAST, "link").map(
    ({ url }) => url
  );
  /**
  ...
  test.each(links)("%s", link => {
    let badLink = !!Object.keys(badLinksAndRecommendations).find(badLink =>
      link.includes(badLink)
    );
    // if blacklisted outdated link but in ignore list, then reset to false
    if (
      badLink &&
      !!oudatedLinksIgnore.find(ignoreLink => link.includes(ignoreLink))
    ) {
      badLink = false;
    }

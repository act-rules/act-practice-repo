...
describe(`Validate link references and definitions`, () => {
  describeRule("Rules", ({ markdownAST }) =>
    validateLinkReferences(markdownAST)
  );
  describePage("Pages", ({ markdownAST }) =>
    validateLinkReferences(markdownAST)
  );
});
...
function validateLinkReferences(markdownAST) {
  ...
  const definitions = uniqueArray(
    getMarkdownAstNodesOfType(markdownAST, "definition").map(({ identifier }) =>

...
function getSpellIgnored() {
  const ignoreConfigured = yaml.safeLoad(
    fs.readFileSync("./__tests__/spelling-ignore.yml", "utf8")
  );
 ...
 */
  ...
  const ignorePrefixes = [
    ...techniquesPrefixes,
    ...techniquesPrefixes.map(t => t.toLowerCase()),
    "sc",
    "usc"
  ];
  let ignoreExtra = [];
   ...
   */
  const ignoreAria = ["aria", ...Array.from(ariaQuery.aria.keys())].map(key =>
    key.replace(/aria-/, "")
  );
